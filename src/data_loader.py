...
def read_feature_data(country, start, stop):
    ...
    combined_df = cleaned_feature_data(combined_df, start, stop)
    combined_df = add_installed_cap(combined_df, country)
    combined_df = combined_df.add_prefix(f"{country}_")

    return combined_df

def add_installed_cap(hourly_df, country):
...
def cleaned_feature_data(df, start, stop, mode='cut'):
...
def read_target_variable(domain, country1, country2):
...
def fix_missing_timestamps(problematic_df, correct_df):
...
def check_timestamps(df1, df2, mode='check'):
    ...
    if problematic_df2 is not None and problematic_df1 is not None:
        print("both dataframes corrupted -> other procedure necessary")
        return
    elif problematic_df2 is None and problematic_df1 is None:
        print("nothing to fix, both dfs are matching")
        return
        
def put_together(df1, df2, df3):
    ...
    y = df_combined["cross_border_capacity"]
    X = df_combined.drop(columns=["cross_border_capacity"])
